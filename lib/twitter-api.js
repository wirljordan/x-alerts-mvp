  ...
  for (const keyword of keywords) {
    if (keyword.startsWith('@')) {
      ...
      if (words.length === 0) {
        ...
        // For multi-word keywords, use more sophisticated matching
        let matchScore = 0
        ...
        for (const word of words) {
          ...
          const wordMatches = wordVariations.some(variation => {
            ...
            return lowerTweetText.includes(lowerVariation) ||
        ...
        // Require higher match percentage for multi-word keywords to avoid false positives
        const requiredMatchPercentage = words.length === 2 ? 0.75 : 0.6 // 75% for 2 words, 60% for 3+ words
        const minimumMatches = Math.ceil(totalWords * requiredMatchPercentage)
        
        if (matchScore >= minimumMatches) {
          // Additional context check to avoid obvious false positives
          if (isRelevantMatch(lowerTweetText, lowerKeyword, words)) {
            return keyword
          }
        }
      }
...
// Helper function to check if a match is contextually relevant
function isRelevantMatch(tweetText, keyword, keywordWords) {
  ...
  // If tweet contains obviously irrelevant content, be more strict
  const hasIrrelevantContent = irrelevantTerms.some(term => tweetText.includes(term))
  
  if (hasIrrelevantContent) {
    // For tweets with irrelevant content, require exact phrase or very high context relevance
    return keywordWords.length === 1 || tweetText.includes(keyword.toLowerCase())
  }
  
  // For business-related keywords, check for business context
  const businessKeywords = ['marketing', 'business', 'growth', 'sales', 'leads', 'customers']
  ...
  if (hasBusinessContext) {
    // Check if tweet has business-related context
    ...
    // For business keywords, require some business context unless it's an exact match
    if (!hasBusinessContextInTweet && !tweetText.includes(keyword.toLowerCase())) {
      return false
    }
  }
