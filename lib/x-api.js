...
export class XAPIClient {
  ...
  async postReply(tweetId, replyText) {
    try {
      ...
      // Try the correct TwitterAPI.io endpoint for posting tweets
      const response = await this.makeRequest('/tweet', {
        method: 'POST',
        body: JSON.stringify({
          text: replyText,
      ...
      try {
        ...
        const altResponse = await this.makeRequest('/twitter/tweet', {
          method: 'POST',
          body: JSON.stringify({
            text: replyText,
            reply_to_tweet_id: tweetId
        ...
        return {
          success: true,
          tweetId: altResponse.data?.id || altResponse.id,
          text: altResponse.data?.text || altResponse.text
        }
      } catch (altError) {
