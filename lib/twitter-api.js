...
// Function to search tweets by multiple keywords in a single request
export async function searchTweetsByMultipleKeywords(keywords, sinceId = null) {
  try {
    // Create OR query from multiple keywords
    const orQuery = keywords.map(keyword => `"${keyword}"`).join(' OR ')
    const query = encodeURIComponent(orQuery)
    ...
    console.log(`🔍 Searching for ${keywords.length} keywords in single request: ${keywords.join(', ')}`)
