...
export async function getXAPIClientForUser(userId, supabase) {
  try {
    ...
    const { data: uuidData, error: uuidError } = await supabase
      .from('users')
      .select('id, x_user_id, twitterapi_login_cookie')
      .eq('id', userId)
      .single()
    ...
    // Check if user has a TwitterAPI.io login cookie
    if (userData.twitterapi_login_cookie) {
      console.log('✅ Using user-specific TwitterAPI.io login cookie')
      return new XAPIClient(userData.twitterapi_login_cookie)
    } else {
      // Fall back to system credentials
      console.log('🔄 No user-specific cookie found, using system TwitterAPI.io credentials')
