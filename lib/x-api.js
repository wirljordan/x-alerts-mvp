...
export async function getTwitterAPIClientForUser(userId, supabase) {
  try {
    // Use system-wide TwitterAPI.io key instead of user-specific credentials
    const systemApiKey = process.env.TWITTER_API_KEY
    
    if (!systemApiKey) {
      throw new Error('TwitterAPI.io API key not configured')
    }
    
    return new TwitterAPIClient(systemApiKey)
  } catch (error) {
